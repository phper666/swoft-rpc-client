 ...
 * @Breaker("order")
 */
class OrderBreaker extends CircuitBreaker
{
    /**
     * The number of successive failures
     * If the arrival, the state switch to open
     *
     * @Value(name="${config.breaker.order.failCount}", env="${ORDER_BREAKER_FAIL_COUNT}")
     * @var int
     */
     ...
     *
     * @Value(name="${config.breaker.order.successCount}", env="${ORDER_BREAKER_SUCCESS_COUNT}")
     * @var int
     */
     ...
     *
     * @Value(name="${config.breaker.order.delayTime}", env="${ORDER_BREAKER_DELAY_TIME}")
     * @var int
     */
